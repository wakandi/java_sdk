...
public class TransactionService extends LedgefarmService {
	...
	public List<Transaction> getAll(int limit, int offset) throws IOException, LedgefarmException, ParseException {
	...
	public List<Transaction> getByWallet(String wallet, int limit, int offset)
			throws IOException, LedgefarmException, ParseException {
		JsonObject responseObject = super.sendHttpGet(null,
				"transaction?limit=" + limit + "&offset=" + offset + "&wallet=" + wallet);
		return this.mapToListObject(responseObject);
	}

	public List<Transaction> getByToken(String token, int limit, int offset)
			throws IOException, LedgefarmException, ParseException {
		JsonObject responseObject = super.sendHttpGet(null,
	...
	public List<Transaction> get(String transactionId) throws IOException, LedgefarmException, ParseException {
	...
	private List<Transaction> mapToListObject(JsonObject jsonObject) throws LedgefarmException, ParseException {
		...
		if (success) {
			...
			for (int i = 0; i < jarray.size(); i++) {
				...
				List<Fee> transfees = new ArrayList<Fee>();
				...
				for (int j = 0; j < assetArray.size(); j++) {
					...
					transfee.setToWallet(feeobject.get("wallet").getAsString());
					transfee.setMemo(feeobject.get("memo").getAsString());
					transfees.add(transfee);
				}
				transaction.setFees(transfees);
